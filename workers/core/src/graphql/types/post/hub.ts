...
builder.mutationFields((t) => ({
	publishPost: t.field({
		...
		resolve: async (_, { input }, ctx) => {
			...
			const library = await ctx.reqCtx.env.VERDANT_LIBRARY.getByName(libraryId);

			if (notebook.coverImageId) {
				const notebookCoverImageInfo = await library.getFileInfo(
					notebook.coverImageId,
				);
				notebookCoverImageUrl = notebookCoverImageInfo?.url ?? null;
			}
			if (notebook.iconId) {
				const notebookIconInfo = await library.getFileInfo(notebook.iconId);
				notebookIconUrl = notebookIconInfo?.url ?? null;
			}
			if (post.coverImageId) {
				const postCoverImageInfo = await library.getFileInfo(post.coverImageId);
			...
			// TODO: invalidate any caches / cdn
	...
	fields: (t) => ({
		...
		url: t.field({
			...
			resolve: (source, _, ctx) => {
				return `${ctx.reqCtx.env.POST_HUB_ORIGIN}/${source.notebookId}/${source.slug}`;
			},
		}),
	}),
});
