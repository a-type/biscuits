...
export async function handleTrialEnd(
	...
	await emailAllAdmins(
		...
		(name) => ({
			...
			text: `Hi ${name},\n\nYour Biscuits trial has ended. You will be charged for your first payment. You can manage (or cancel) your plan at ${ctx.env.UI_ORIGIN}/plan. Please contact support if you have any questions.\n\nThanks,\nGrant`,
			html: `<div>
          <p>Hi ${name},</p>
          <p>Your Biscuits trial has ended. You will be charged for your first payment. You can manage (or cancel) your plan at <a href="${ctx.env.UI_ORIGIN}/plan">${ctx.env.UI_ORIGIN}/plan</a>.</p>
...
export async function handleSubscriptionCreated(
	...
	const productMetadata = await getProductMetadata(productId, stripe);
	...
	if (
		...
		await emailAllAdmins(
			...
			(name) => ({
				...
				text: `Hi ${name},\n\nYour Biscuits free trial has started! You can manage (or cancel) your plan at ${ctx.env.UI_ORIGIN}/plan. Please contact support if you have any questions.\n\nThanks,\nGrant`,
				html: `<div>
      <h1>Your Biscuits free trial has started!</h1>
      <p>Hi ${name},</p>
      <p>Your Biscuits free trial has started! You can manage (or cancel) your plan at <a href="${ctx.env.UI_ORIGIN}/plan">${ctx.env.UI_ORIGIN}/plan</a>. You won't be charged until ${
	...
	if (!!canceledAt) {
		...
		await emailAllAdmins(
            ...
            <p>Your Biscuits subscription was cancelled. Your plan will remain active until ${
						...
						}. Restart your subscription anytime at <a href="${ctx.env.UI_ORIGIN}/plan">${ctx.env.UI_ORIGIN}/plan</a>.</p>
            <p>Please contact support if you have any questions.</p>
            <p>Thanks,</p>
            <p>Grant</p>
          </div>`,
		...
		if (members.length > productMetadata.memberLimit) {
			...
			await emailAllAdmins(
				...
				(name) => ({
					...
					text: `Hi ${name},\n\nYou recently changed your Biscuits subscription, and now it has more than the allowed number of members. Please visit ${ctx.env.UI_ORIGIN}/plan to remove extra members, or your subscription will remain paused.\n\nThanks,\nGrant`,
					html: `<div>
          <p>Hi ${name},</p>
          <p>You recently changed your Biscuits subscription, and now it has more than the allowed number of members. Please visit <a href="${ctx.env.UI_ORIGIN}/plan">${ctx.env.UI_ORIGIN}/plan</a> to remove extra members, or your subscription will remain paused.</p>
          <p>If you think this was a mistake, don't hesitate to reach out via <a href="${ctx.env.UI_ORIGIN}/contact">${ctx.env.UI_ORIGIN}/contact</a>.</p>
          <p>Thanks,</p>
          <p>Grant</p>
        </div>`,
				}),
...
export async function handlePaymentIntentSucceeded(
	...
	const stripe = getStripe(ctx.env.STRIPE_SECRET_KEY);
	const subscription = await getSubscriptionIdOfPaymentIntent(
		paymentIntent,
		stripe,
	);
	if (!subscription) {
...
export async function handlePaymentIntentCanceled(
	...
	await emailAllAdmins(
		...
		(name) => ({
			...
			html: `<div>
          <p>Hi ${name},</p>
          <p>Your Biscuits subscription payment was cancelled. If this was a mistake, try checking out again at <a href="${ctx.env.UI_ORIGIN}/plan">${ctx.env.UI_ORIGIN}/plan</a>.</p>
          <p>Please contact support if you have any questions.</p>
          <p>Thanks,</p>
          <p>Grant</p>
          </div>`,
...
export async function handlePaymentIntentPaymentFailed(
	...
	await emailAllAdmins(
		...
		(name) => ({
			...
			html: `<div>
          <p>Hi ${name},</p>
          <p>Your Biscuits subscription payment failed. If this was a mistake, try checking out again at <a href="${ctx.env.UI_ORIGIN}/plan">${ctx.env.UI_ORIGIN}/plan</a>.</p>
          <p>Please contact support if you have any questions.</p>
          <p>Thanks,</p>
          <p>Grant</p>
          </div>`,
