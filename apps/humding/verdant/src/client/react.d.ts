...
export interface GeneratedHooks<Presence, Profile> {
  ...
  useSong(id: string, config?: { skip?: boolean }): Song | null;
  useSongUnsuspended(
    id: string,
    config?: { skip?: boolean },
  ): { data: Song | null; status: QueryStatus };
  useOneSong: <Config extends HookConfig<SongFilter>>(
    config?: Config,
  ) => Song | null;
  useOneSongsUnsuspended: <Config extends HookConfig<SongFilter>>(
    config?: Config,
  ) => { data: Song | null; status: QueryStatus };
  useAllSongs: <Config extends HookConfig<SongFilter>>(
    config?: Config,
  ) => Song[];
  useAllSongsUnsuspended: <Config extends HookConfig<SongFilter>>(
    config?: Config,
  ) => { data: Song[]; status: QueryStatus };
  useAllSongsPaginated: <
    Config extends HookConfig<SongFilter> & {
      pageSize?: number;
      suspend?: false;
    },
  >(
