...
export class ScanForbiddenError extends Error {
  constructor() {
    super("Sorry, this website doesn't allow Wish Wash to scan product info.");
  }
}

export async function scanStorePage(siteUrl: string) {
  ...
  if (robotsFileResponse.status !== 404) {
    ...
    try {
      ...
      if (robots?.isDisallowed(siteUrl, 'WishWashBot')) {
        return null;
      }
    } catch (e) {
