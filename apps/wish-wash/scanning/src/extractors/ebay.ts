import { CheerioAPI } from 'cheerio';
import { ExtractorData } from './types.js';

export async function ebay(
	$: CheerioAPI,
	_pageUrl: string,
): Promise<ExtractorData | null> {
	const productName = $('[data-test-id="itemTitle"]').text().trim();
	const price = $('[data-test-id="item-price"]').text().trim();
	const image = $('[data-test-id="image-carousel"]').find('img').attr('src');
	const priceNumber = price.match(/\d+\.\d+/g);
	const priceValue = priceNumber ? priceNumber[0] : undefined;
	const priceSymbol = price
		.match(/[^\d.]/g)
		?.join('')
		.trim();
	return {
		scanner: 'ebay',
		productName: productName || undefined,
		price: priceValue,
		currency: priceSymbol || undefined,
		priceIsVariable: price?.endsWith('+'),
		imageUrl: image,
	};
}
