...
export async function tasty($: CheerioAPI): Promise<ExtractorData | null> {
	...
	const prepTimeMinutes =
		prepTimeElement ? humanTimeToMinutes(prepTimeElement.text()) : null;
	const cookTimeMinutes =
		cookTimeElement ? humanTimeToMinutes(cookTimeElement.text()) : null;
	const totalTimeMinutes =
		totalTimeElement ? humanTimeToMinutes(totalTimeElement.text()) : null;
